...
export class JetpackAuthorize extends Component {
	static propTypes = {
		...
		isSiteBlocklisted: PropTypes.bool,
	...
	renderNotices() {
		...
		if ( this.props.isSiteBlocklisted ) {
			return (
				<JetpackConnectNotices
					noticeType={ SITE_BLOCKLISTED }
					onTerminalError={ redirectToMobileApp }
				/>
			);
		}
...
const connectComponent = connect(
	( state, { authQuery } ) => {
		...
		return {
			...
			isMobileAppFlow,
			isSiteBlocklisted: isSiteBlocklistedSelector( state ),
